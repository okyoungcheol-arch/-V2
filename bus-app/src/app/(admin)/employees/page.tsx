...
export default function EmployeesPage() {
  ...
  const columns: Column<Employee>[] = [
    ...
    { key: 'phone', label: '연락처' },
    { key: 'login_id', label: '로그인ID' },
    {
      key: 'monthly_salary', label: '월급여',
      render: (v) => Number(v) > 0 ? Number(v).toLocaleString() + '원' : '-',
    },
    {
      key: 'can_drive', label: '운전가능',
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <Field label="로그인 ID">
                  <input className={ic} value={form.login_id ?? ''} onChange={(e) => setForm({ ...form, login_id: e.target.value })} placeholder="로그인 아이디" />
                </Field>
